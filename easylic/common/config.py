...
class Config:
    ...
    REQUIRED_FEATURES: ClassVar[dict[str, bool]] = {
    ...
    SERVER_KEYS_DIR: Path = Path(
        os.getenv("EASYLIC_KEYS_DIR", str(BASE_DIR / "server"))
    )
    ...
    @classmethod
    def get_server_keys(cls) -> tuple[Ed25519PublicKey, Ed25519PrivateKey]:
        """Load server keys from files."""
        try:
            with cls.SERVER_PUBLIC_KEY_PATH.open("rb") as f:
                server_pub = cast(
                    "Ed25519PublicKey", serialization.load_pem_public_key(f.read())
                )
            with cls.SERVER_PRIVATE_KEY_PATH.open("rb") as f:
                server_priv = cast(
                    "Ed25519PrivateKey",
